...
public class RegisterService {
    ...
    public String register(PhoneRegisterVo info) {
    ...
    public void sendEmailMessage(String email) {
        //校验手机号输入是否正确
        if (!FgrUtil.checkEmail(email)){
            throw new ResultException("邮箱输入不正确");
        }
        //获取六位验证码
        String code = FgrUtil.getCode();
        if (messageUtil.sendThymeleafMail(email,code)){
            //根据手机号发送短信
    ...
    public String registerOfEmail(EmailRegisterVo info) {
        String email = info.getEmail();
        //校验验证码是否正确
        String captcha = redisCache.getCacheObject(email);
        if (captcha == null){
            throw new ResultException("验证码过期");
        }
        if (!info.getCode().equalsIgnoreCase(captcha)){
        ...
        if (ObjectUtil.isNull(userDo)){
            ...
            userDo.setNickName("email_"+email);
